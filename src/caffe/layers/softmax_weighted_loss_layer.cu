...
__global__ void SoftmaxWeightedLossForwardGPU(const int nthreads,
          ...
          const Dtype *sample_weight, Dtype* loss,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    const Dtype w = sample_weight[n * spatial_dim + s];
...
template <typename Dtype>
void SoftmaxWeightedLossLayer<Dtype>::Forward_gpu(
  ...
  SoftmaxWeightedLossForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      CAFFE_CUDA_NUM_THREADS>>>(nthreads, prob_data, label,
          sample_weight_blob->gpu_data(), loss_data, outer_num_, dim,
          inner_num_, has_ignore_label_, ignore_label_, counts);
  Dtype loss;
...
__global__ void SoftmaxWeightedLossBackwardGPU(const int nthreads, const Dtype* top,
          const Dtype* label, const Dtype *sample_weight, Dtype* bottom_diff,
          const int num, const int dim, const int spatial_dim,
          const bool has_ignore_label_, const int ignore_label_,
          Dtype* counts) {
...
template <typename Dtype>
void SoftmaxWeightedLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    SoftmaxWeightedLossBackwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, top_data, label,
            sample_weight_blob->gpu_data(), bottom_diff, outer_num_, dim,
            inner_num_, has_ignore_label_, ignore_label_, counts);
...
INSTANTIATE_LAYER_GPU_FUNCS(SoftmaxWeightedLossLayer);
